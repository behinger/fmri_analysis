...
cfg.bidsdir = fullfile('/','project','3018029.10','sustained','data','pilot','bids');

SID = 'sub-05'
niftis = [dir(fullfile(cfg.bidsdir,'derivates','preprocessing',SID,'ses-01','func','*task-sus*run-*Realign_bold.nii'))];
mask_varea = dir(fullfile(cfg.bidsdir,'derivates','preprocessing',SID,'ses-01','label','*desc-varea_space-FUNCCROPPED_label.nii'));
mask_eccen = dir(fullfile(cfg.bidsdir,'derivates','preprocessing',SID,'ses-01','label','*desc-eccen_space-FUNCCROPPED_label.nii'));
...
%% Load Event Files
eventFiles = [dir(fullfile(cfg.bidsdir,SID,'ses-01','func','*task-sus*run-*events.tsv'))];
events = [];
