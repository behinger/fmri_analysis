function argout= ndgrid_cell(varargin)
% Modified matlab function to return a cell array instead of varargout
%
...
argout = cell(1,max(length(varargin),1));
if nout == 2 % Optimized Case for 2 dimensions
    x = full(varargin{j(1)}(:));
    y = full(varargin{j(2)}(:)).';
    argout{1} = repmat(x,size(y));
    argout{2} = repmat(y,size(x));
else
    for i=1:max(length(varargin),1)
        x = full(varargin{j(i)});
        s = ones(1,nout); 
        s(i) = numel(x);
        x = reshape(x,s);
