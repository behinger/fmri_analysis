function calc_spm2ndLevel_new(bidsdir,subjectlist,events,varargin)

cfg = finputcheck(varargin, ...
    {
    'task'             'string',   {'localizer','WM'}    [];... % rois from benson17 (V1=1,V2=2,V3=3)
    'TR','real',[],[];
    'conditions','cell',[],{'orientation','contrast'} % specifies the factorial design
            ...
            for run = unique(events.run)'
                ...
                for ci = 1:numel(combinations{1})
                    ...
                    for c = 1:length(cfg.conditions)
                        if iscell(events{1,cfg.conditions{c}})
                            ...
                            name = [name '_' cfg.conditions{c} ':' num2str(conditionLevels{c}(combinations{c}(ci)))];
