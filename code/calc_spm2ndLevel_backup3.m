...
for SID = subjectlist
    ...
    events = events([1:56],:); %for the first pilot
    niftis = [dir(fullfile(datadir,'derivates','preprocessing',SID,'ses-01','func',[SID,'_ses-01_task-',cfg.task,'_desc-occipitalcropRealign_bold.nii']))];
...
%                     for contrast = unique(events.contrast)'
        ...
        matlabbatch{3}.spm.stats.con.consess{1}.tcon.weights = [0 1 1 1 1];
